...
export function generateDungeon(seed,{tier=1,size=31}={}){
 ...
 const candidates=shuffle(floors.filter(p=>p.d>5&&!reserved.has(key(p.x,p.y))),rng),enemies=[],traps=[],caches=[],shrines=[];
 const takeCandidate=(predicate=()=>true)=>{const i=candidates.findIndex(p=>!reserved.has(key(p.x,p.y))&&predicate(p));if(i<0)return null;const [p]=candidates.splice(i,1);reserved.add(key(p.x,p.y));return p;};
 const relicPoint=takeCandidate(p=>p.d>Math.floor(size*.62))||takeCandidate(),relic=relicPoint?{x:relicPoint.x,y:relicPoint.y,claimed:false}:null;
 for(const kind of SHRINE_KINDS){const p=takeCandidate(q=>q.d>7);if(p)shrines.push({kind,x:p.x,y:p.y,used:false});}
 for(let i=0;i<4+tier;i++){const p=takeCandidate();if(!p)break;enemies.push({id:`warden-${i+1}`,name:['Ash Warden','Stone Shade','Vault Sentinel','Hollow Guard'][i%4],x:p.x,y:p.y,hp:18+tier*6,maxHp:18+tier*6,attack:5+tier*2,alive:true});}
 for(let i=0;i<6+tier*2;i++){const p=takeCandidate();if(!p)break;traps.push({x:p.x,y:p.y,damage:8+Math.floor(rng()*(8+tier*2)),triggered:false});}
 for(let i=0;i<3;i++){const p=takeCandidate();if(!p)break;caches.push({x:p.x,y:p.y,opened:false,item:LOOT[Math.floor(rng()*LOOT.length)],qty:1+Math.floor(rng()*(2+tier))});}
 const run={active:true,id:`dungeon-${hashSeed(seed).toString(16)}`,seed:String(seed),tier,width:size,height:size,map:grid.map(r=>r.join('')),position:{...start},start,exit,seals,enemies,traps,caches,shrines,relic,health:100,maxHealth:100,steps:0,kills:0,randomCounter:0,discovered:[],log:[],startedAt:Date.now()};reveal(run,start.x,start.y,3);log(run,'The stone door closes behind you. Find the three rune seals, then reach the exit gate. A relic vault lies somewhere off the safe path.');return run;
}
...
function finishRun(w,p,status,{transfer,event}={}){
 ...
 if(status==='success'){
  stats.completions++;stats.bestSteps=stats.bestSteps==null?run.steps:Math.min(stats.bestSteps,run.steps);const relicRecovered=!!run.relic?.claimed,bounty=150+run.tier*75+(relicRecovered?75+run.tier*25:0),coinReward=Math.min(bounty,w.treasury.cash),paid=coinReward>0&&(transfer?.(w,'treasury',p.id,coinReward,'dungeon-bounty')||false);const reward={coins:paid?coinReward:0,ore:2+run.tier,goods:1+Math.floor(run.tier/2),artifact:relicRecovered?1:0};p.inventory.ore=(p.inventory.ore||0)+reward.ore;p.inventory.goods=(p.inventory.goods||0)+reward.goods;if(reward.artifact)p.inventory.artifact=(p.inventory.artifact||0)+reward.artifact;w.produced.ore=(w.produced.ore||0)+reward.ore;w.produced.goods=(w.produced.goods||0)+reward.goods;if(reward.artifact)w.produced.artifact=(w.produced.artifact||0)+reward.artifact;p.skills??={};p.skills.adventure=(p.skills.adventure||0)+(relicRecovered?2:1);p.lastDungeonResult={status:'success',tick:w.tick,steps:run.steps,tier:run.tier,reward};event?.(w,'dungeon',`${p.name} clears the Copperhill Catacombs`,`Three rune seals opened the final gate. The expedition returned with ${reward.ore} ore, ${reward.goods} goods${reward.artifact?', an old-world artifact':''}${reward.coins?` and ${reward.coins} coins`:''}.`,[p.id]);
 }else if(status==='death'){
  stats.deaths++;const coinLoss=Math.min(500,Math.floor((p.cash||0)*.12));if(coinLoss>0)transfer?.(w,p.id,'treasury',coinLoss,'dungeon-recovery');const items=Object.keys(p.inventory||{}).filter(k=>(p.inventory[k]||0)>0);let lostItem=null;if(items.length){lostItem=items[hashSeed(`${run.seed}:${run.steps}`)%items.length];p.inventory[lostItem]--;w.consumed[lostItem]=(w.consumed[lostItem]||0)+1;}p.health=Math.max(35,(p.health||80)-12);p.needs??={};p.needs.energy=Math.min(p.needs.energy??50,20);p.lastDungeonResult={status:'death',tick:w.tick,steps:run.steps,tier:run.tier,lostCoins:coinLoss,lostItem,relicLost:!!run.relic?.claimed};event?.(w,'dungeon',`${p.name} is carried out of the catacombs`,`The expedition failed after ${run.steps} steps. Recovery cost ${coinLoss} coins${lostItem?` and one ${lostItem}`:''}${run.relic?.claimed?' and the relic was lost':''}.`,[p.id]);
 }else{stats.abandons++;p.lastDungeonResult={status:'abandoned',tick:w.tick,steps:run.steps,tier:run.tier,relicLost:!!run.relic?.claimed};}
...
function applyTile(w,p,run,helpers){
 ...
 const shrine=run.shrines?.find(s=>!s.used&&s.x===x&&s.y===y);if(shrine){shrine.used=true;if(shrine.kind==='mending'){const before=run.health;run.health=Math.min(run.maxHealth,run.health+32+run.tier*3);log(run,`Mending shrine restored ${Math.round(run.health-before)} health.`);}else{const label=revealNearestObjective(run,x,y);log(run,label?`Wayfinder shrine revealed a nearby ${label}.`:'Wayfinder shrine hums, but every objective is already known.');}}
 if(run.relic&&!run.relic.claimed&&run.relic.x===x&&run.relic.y===y){run.relic.claimed=true;log(run,'The relic vault opens. Clear the dungeon to carry the artifact home.');}
