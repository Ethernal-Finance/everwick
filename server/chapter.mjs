...
function collect(w,p,transfer){near(p,9,28);const farm=w.businesses.find(b=>b.id==='sunfield');if(farm.inventory.food<5)throw Error('The farm needs to grow more food. Try again after production.');if(!transfer(w,'treasury',farm.id,5*farm.price,'town-supply-contract'))throw Error('The town cannot fund a shipment yet');farm.inventory.food-=5;farm.revenue+=5*farm.price;farm.soldToday=(farm.soldToday||0)+5;p.contract={food:5};}
function deliver(w,p,reward,transfer){near(p,29,29);if(!p.contract)throw Error('Collect a shipment first');if(!transfer(w,'treasury',p.id,reward,'delivery-reward'))throw Error('Town payroll is short. Your shipment is safe; retry later.');const clinic=w.businesses.find(b=>b.id==='clinic');clinic.inventory.food=(clinic.inventory.food||0)+p.contract.food;p.contract=null;p.lastDelivery=w.tick;}
